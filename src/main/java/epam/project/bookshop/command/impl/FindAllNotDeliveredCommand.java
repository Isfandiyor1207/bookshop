...
public class FindAllNotDeliveredCommand implements Command {
    ...
    public String execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            List<OrderDto> orderByUser = orderService.findAllNotDeliveredOrder();

            if (orderByUser.size() == 0) {
                request.setAttribute(WARN_USER_IS_NOT_ORDERED, ERROR_USER_IS_NOT_ORDERED);
            } else {
                request.setAttribute(DELIVERED_STATUS, true);
